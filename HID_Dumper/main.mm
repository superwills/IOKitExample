...
void runLoopPolling_NotRecommended( HIDManager& hid )
{
  // 2) Input polling
  while( 1 )
  {
//    if( ioCheck( IOHIDDeviceGetReport( hid.mouse.dev, IOHIDReportType::kIOHIDReportTypeInput,
//      hid.mouse.x.reportId, report, &realReportLen ), "get mouse report" ) )
//    {
//      printf( "%d%d", report[0], report[1] );
//    }
...
int main( int argc, const char *	argv[] )
{
  ...
  // 2) A while(1) style loop with input polling
  ///runLoopPolling_NotRecommended(hid);
